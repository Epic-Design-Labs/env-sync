import fs from 'node:fs';
import path from 'node:path';
import { runAdd } from './add.js';
import { CONFIG_FILE, readConfig } from './config.js';
import { CliError, EXIT } from './errors.js';
import { repoRoot, requireIgnored, scratchDir } from './fsutil.js';
import { Op } from './op.js';
import { runPull } from './pull.js';
import { runSetup } from './setup.js';

const pkg = JSON.parse(fs.readFileSync(new URL('../package.json', import.meta.url), 'utf8'));

const USAGE = `env-sync ${pkg.version} — render .env files from 1Password, matched by variable name

  env-sync pull   [--yes] [--dry-run] [--only <dest>]   write .env files from 1Password (default)
  env-sync check  [--only <dest>]                       exit 1 if any .env is out of date
  env-sync add KEY [--to <template>] [--app]            add a new secret to 1Password and the template
  env-sync setup  [--rehearse] [--vault <name>] [--account <name>] [--source <dir>]
                  [--include KEY@path] [--force]        one-time: move a project's values into 1Password

Exit codes: 0 ok · 1 out of date · 2 op missing or signed out · 3 vault unreachable
            4 not in 1Password · 5 bad config or arguments`;

const FLAGS = {
  pull: { yes: 'bool', y: 'yes', 'dry-run': 'bool', only: 'str' },
  check: { only: 'str' },
  add: { to: 'str', app: 'bool' },
  setup: { rehearse: 'bool', scratch: 'rehearse', vault: 'str', account: 'str', source: 'str', include: 'list', force: 'bool' },
};

function parseArgs(cmd, args) {
  const spec = FLAGS[cmd];
  const out = { _: [], include: [] };
  for (let i = 0; i < args.length; i++) {
    const a = args[i];
    if (!a.startsWith('-')) { out._.push(a); continue; }
    let name = a.replace(/^--?/, '');
    let type = spec[name];
    if (type && !['bool', 'str', 'list'].includes(type)) { name = type; type = spec[name]; }
    if (!type) throw new CliError(EXIT.USAGE, `unknown option ${a} for "${cmd}"\n\n${USAGE}`);
    if (type === 'bool') { out[name] = true; continue; }
    const v = args[++i];
    if (v === undefined || v.startsWith('-')) throw new CliError(EXIT.USAGE, `${a} needs a value`);
    if (type === 'list') out[name].push(v); else out[name] = v;
  }
  return out;
}

function requireOp() {
  if (new Op({ vault: '' }).run(['--version']).status !== 0) {
    throw new CliError(EXIT.NO_OP, '1Password CLI (op) is not working. Install it: brew install 1password-cli');
  }
}

/** Setup may fix the containment rule itself; pull/check/add only insist on it. */
function ensureIgnored(root, log) {
  try { requireIgnored(root); return; } catch { /* add it below */ }
  const gi = path.join(root, '.gitignore');
  const cur = fs.existsSync(gi) ? fs.readFileSync(gi, 'utf8') : '';
  fs.writeFileSync(gi, cur.replace(/\n?$/, cur ? '\n' : '') +
    '\n# env-sync renders secrets into a scratch dir here; never commit it.\n.env-sync.*\n');
  log.out("Added '.env-sync.*' to .gitignore\n");
  requireIgnored(root);
}

export async function main(argv) {
  const log = {
    out: (s) => process.stdout.write(`${s}\n`),
    warn: (s) => process.stderr.write(`warning: ${s}\n`),
  };
  try {
    const first = argv[0];
    if (first === '--version' || first === '-v') { log.out(pkg.version); return EXIT.OK; }
    if (first === 'help' || first === '--help' || first === '-h') { log.out(USAGE); return EXIT.OK; }
    const cmd = first && !first.startsWith('-') ? first : 'pull';
    if (!FLAGS[cmd]) throw new CliError(EXIT.USAGE, `unknown command "${cmd}"\n\n${USAGE}`);
    const opts = parseArgs(cmd, first === cmd ? argv.slice(1) : argv);

    requireOp();
    const root = repoRoot();

    if (cmd === 'setup') {
      const cfg = fs.existsSync(path.join(root, CONFIG_FILE)) ? readConfig(root) : null;
      const vault = opts.vault ?? cfg?.vault;
      if (!vault) throw new CliError(EXIT.USAGE, 'which vault? pass --vault "<Project> Dev" (or set vault in env-sync.conf)');
      ensureIgnored(root, log);
      return runSetup({
        root, cfg, vault, account: opts.account ?? cfg?.account ?? null,
        source: fs.realpathSync(path.resolve(opts.source ?? root)),
        rehearse: !!opts.rehearse, force: !!opts.force, includes: new Set(opts.include),
        scratch: scratchDir(root), log,
      });
    }

    const cfg = readConfig(root);
    requireIgnored(root);
    const scratch = scratchDir(root);
    const op = new Op({ account: cfg.account, vault: cfg.vault, scratch });
    op.preflight();

    if (cmd === 'add') return runAdd({ cfg, op, key: opts._[0], to: opts.to, app: !!opts.app, log });
    const code = runPull(cmd, { cfg, op, scratch, only: opts.only, yes: !!opts.yes, dryRun: !!opts['dry-run'], log });
    if (code === EXIT.DRIFT) log.out('Out of date. Run: pnpm env:pull');
    return code;
  } catch (e) {
    if (e instanceof CliError) { process.stderr.write(`env-sync: ${e.message}\n`); return e.code; }
    // Unexpected errors can carry data from op's output, so details are opt-in.
    if (process.env.ENV_SYNC_DEBUG) process.stderr.write(`${e.stack}\n`);
    else process.stderr.write(`env-sync: unexpected ${e.name}. Re-run with ENV_SYNC_DEBUG=1 for details.\n`);
    return EXIT.DRIFT;
  }
}
